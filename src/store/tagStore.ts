    ...
    (set, get) => ({
      ...
      removeTag: (tagId: string) => {
        set(state => {
          ...
          const newRecentTags = state.recentTags.filter((id): boolean => id !== tagId);
          const newFavoriteTags = state.favoriteTags.filter((id): boolean => id !== tagId);
      ...
      getTagsByCategory: (category: TagCategory): TagMetadata[] => {
        const state = get();
        return Array.from(state.tags.values()).filter((tag): boolean => tag.category === category);
      },
      ...
      removeFilterPreset: (presetId: string) => {
        set(state => {
          const newPresets = state.filterPresets.filter((p): boolean => p.id !== presetId);
          return { filterPresets: newPresets };
        });
      },
      ...
      searchTags: (query: string): TagMetadata[] => {
        ...
        return Array.from(state.tags.values()).filter(
          (tag): boolean =>
            ...
            Boolean(tag.aliases?.some((alias): boolean => alias.toLowerCase().includes(lowerQuery)))
        );
      },
      ...
      generateSuggestions: (messageContent: string, existingTags: string[]): TagSuggestion[] => {
        ...
        state.tags.forEach((tag): void => {
          ...
          if (tag.description) {
            ...
            const matchingKeywords = tagKeywords.filter((kw): boolean => keywords.includes(kw));
    ...
    }),
    {
      ...
      onRehydrateStorage: () => state => {
        if (state) {
          ...
          state.usageStats = new Map(
            state.usageStats as unknown as Iterable<readonly [string, TagUsageStats]>
          );
        }
      },
    }
