...
export const RESPONSE_SIZE_PRESETS: ResponseSizePreset[] = [
  {
    ...
    tokens: 512,
    icon: '📝',
    useCases: ['Quick Q&A', 'Legal definitions', 'Status updates', 'Simple explanations'],
  },
  {
    id: 'standard',
    name: 'Standard',
    ...
    tokens: 2048,
    icon: '📄',
    useCases: ['Legal analysis', 'Research summaries', 'Email drafting', 'General consultation'],
  },
  {
    id: 'extended',
    name: 'Extended',
    ...
    tokens: 4096,
    icon: '📋',
    useCases: ['Contract analysis', 'Case summaries', 'Detailed research', 'Multi-point arguments'],
  },
  {
    id: 'maximum',
    name: 'Maximum',
 ...
 */
export function constrainMaxTokens(requestedTokens: number, modelMaxMaxTokens: number): number {
 ...
 */
export function getRecommendedPreset(modelMaxMaxTokens: number): ResponseSizePreset {
  // If model supports ultra or extreme, default to extended for safety
  if (modelMaxMaxTokens >= RESPONSE_SIZE_PRESETS[5].tokens) {
    // >= 16384
    return RESPONSE_SIZE_PRESETS[2]; // Extended (4096)
  }

  // If model supports maximum or extended, default to standard
  if (modelMaxMaxTokens >= RESPONSE_SIZE_PRESETS[3].tokens) {
    // >= 8192
...
export function getAvailablePresets(modelMaxMaxTokens: number): ResponseSizePreset[] {
  return RESPONSE_SIZE_PRESETS.filter((preset): boolean => preset.tokens <= modelMaxMaxTokens);
}
 ...
 */
export function getSelectionDescription(currentTokens: number, modelMaxMaxTokens: number): string {
