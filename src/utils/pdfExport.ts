...
function addPDFHeader(pdf: jsPDF, conversation: Conversation, margin: number, pageWidth: number): number {
  ...
  // Extract practice areas, advisory areas, and jurisdictions from messages if not on conversation
  // Try conversation first (may not be set), then fall back to first message
  let practiceAreas: string[] = [];
  let advisoryArea: string | undefined;
  ...
  if (conversation.selectedJurisdictions && conversation.selectedJurisdictions.length > 0) {
    jurisdictions = conversation.selectedJurisdictions;
  }
  ...
  if (practiceAreas.length > 0) {
    pdf.text(`Practice Area: ${practiceAreas.join(' & ')}`, margin, yPosition);
    yPosition += 5;
  }

  // Show advisory area
  if (advisoryArea) {
    pdf.text(`Advisory Area: ${advisoryArea}`, margin, yPosition);
    yPosition += 5;
  }

  // Show jurisdictions if specified
  if (jurisdictions.length > 0) {
    pdf.text(`Jurisdictions: ${jurisdictions.join(', ')}`, margin, yPosition);
    yPosition += 5;
  }
...
function addMessageContent(pdf: jsPDF, message: Message, margin: number, maxWidth: number, pageWidth: number, pageHeight: number, startY: number): number {
  ...
  if (message.practiceArea || message.otherPracticeArea || message.advisoryArea) {
    ...
    if (message.advisoryArea) {
      if (areaText) {
        areaText += ` | Advisory Area: ${message.advisoryArea}`;
      } else {
        areaText = `Advisory Area: ${message.advisoryArea}`;
      }
    }
...
export async function exportConversationToPDF(conversation: Conversation): Promise<string> {
  ...
  for (let i = 0; i < conversation.messages.length; i++) {
    ...
    // Start each assistant response on a new page for better readability
    // This ensures each answer is cleanly separated and easier to comprehend
    if (message.role === 'assistant' && prevMessage?.role === 'user') {
      pdf.addPage();
      yPosition = margin;
    }

    // Start analysis messages on a new page with a clear header
    if (isAnalysis && i > 0) {
      pdf.addPage();
      yPosition = margin;

      // Add visual header for analysis section
      pdf.setFillColor(255, 250, 240);
      pdf.rect(0, yPosition - 5, pageWidth, 20, 'F');
      pdf.setDrawColor(255, 200, 100);
      pdf.setLineWidth(0.5);
      pdf.line(margin, yPosition + 13, pageWidth - margin, yPosition + 13);

      pdf.setFontSize(14);
      pdf.setFont('helvetica', 'bold');
      pdf.setTextColor(180, 100, 0);
      pdf.text('RESPONSE ANALYSIS', margin, yPosition + 8);
      pdf.setTextColor(0, 0, 0);
      yPosition += 25;
    }
...
export async function exportClusterToPDF(
  ...
  // Extract metadata from first message (typically the user query)
