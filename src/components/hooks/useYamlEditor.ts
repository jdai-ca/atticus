...
interface UseYamlEditorResult {
  ...
  setKeywordInputs: React.Dispatch<
...
export function useYamlEditor(): UseYamlEditorResult {
  ...
  const resetToFactory = async (
    ...
    try {
      setIsResetting(type);
      const result = await globalThis.window.electronAPI.fetchFactoryConfig(
        filename,
      );
  ...
  const loadYamlContent = async (
    ...
    try {
      ...
      const result = await globalThis.window.electronAPI.loadBundledConfig(
        filename,
      );
  ...
  const removeKeywordFromArea = (index: number, keywordIndex: number): void => {
    ...
    updated[index] = {
      ...
      keywords: updated[index].keywords.filter(
        (_: string, i: number) => i !== keywordIndex,
      ),
    };
    setParsedAreas(updated);
  };
