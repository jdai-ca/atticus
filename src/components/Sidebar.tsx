...
import { MessageSquare, Plus, Trash2, FileText, Edit2, Check, X, Search } from 'lucide-react';
...
export default function Sidebar({ onOpenSettings, onNewConversation }: SidebarProps) {
  ...
  const handleNewChat = () => {
    if (!config.activeProviderId) {
      alert(t.alerts.configureProviderFirst);
      onOpenSettings();
      return;
    }
                  ...
                  {editingId === conv.id ? (
                    <fieldset
                      className={`w-full p-3 rounded-lg transition-colors border-0 ${
                        currentConversation?.id === conv.id ? 'bg-gray-700' : 'hover:bg-gray-700/50'
                      }`}
                    >
                      ...
                      <div className="flex items-start gap-2">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {conv.practiceArea && (
                            <div className="text-xs text-gray-400 mt-1">{conv.practiceArea}</div>
                          )}
                        </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-gray-800 rounded-lg w-full max-w-md mx-4 border border-gray-700">
            ...
            <div className="flex items-center justify-between p-4 border-b border-gray-700">
              <h3 className="text-lg font-semibold text-white">{t.searchConversations}</h3>
              <button
                onClick={() => {
            ...
            <div className="p-4 border-b border-gray-700">
              ...
              <div className="text-xs text-gray-500 mt-2 px-1">
                Tip: Use #corporate-law, #strategic-planning (areas), or #important,
                #contract-review (custom tags)
              </div>
            </div>
                ...
                .filter((conv): boolean => {
                  ...
                  if (query.startsWith('#')) {
                    ...
                    return (
                      ...
                      conv.tags?.some((tag): boolean => tag.toLowerCase() === tagId) ||
                      conv.messages.some((msg): boolean =>
                        Boolean(msg.tags?.some((tag): boolean => tag.toLowerCase() === tagId))
                      )
                    );
                  }
                  ...
                  return (
                    ...
                    conv.messages.some((msg): boolean => msg.content.toLowerCase().includes(query))
                  );
                })
                .map(
                  (conv): JSX.Element => (
