...
import { TagMetadata, TagCategory, TagSuggestion, TagAutocompleteResult } from '../types/tags';
...
const TAG_CATEGORY_CONFIG: Record<TagCategory, { label: string; icon: string; color: string }> = {
...
export const TagManager: React.FC<TagManagerProps> = ({
  ...
  const [autocompleteResults, setAutocompleteResults] = useState<TagAutocompleteResult[]>([]);
  ...
  useEffect(() => {
    ...
    allAvailableTags.forEach((tag): void => {
      ...
      // Prefix match
      else if (tag.id.startsWith(query) || tag.label.toLowerCase().startsWith(query)) {
        matchType = 'prefix';
        relevanceScore = 0.8;
      }
      // Alias match
      else if (tag.aliases?.some((alias): boolean => alias.toLowerCase().includes(query))) {
        matchType = 'alias';
        relevanceScore = 0.7;
      }
      // Fuzzy match (contains)
      else if (tag.id.includes(query) || tag.label.toLowerCase().includes(query)) {
        matchType = 'fuzzy';
        relevanceScore = 0.5;
      }
    ...
    const hasExactMatch = results.some((r): boolean => r.matchType === 'exact');
  ...
  const getFilteredTags = () => {
    ...
    return tagsArray.filter(
      (t): boolean => existingTags.includes(t.id) && t.category === selectedCategory
    );
  };

  // Group suggestions by confidence
  const highConfidenceSuggestions = suggestedTags.filter(s => s.confidence >= 0.7);
    ...
    <div className="bg-gray-800 rounded-lg border border-gray-700 shadow-2xl overflow-hidden">
                  ...
                  >
                    <div className="flex items-center gap-2 flex-1 min-w-0">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-sm font-medium text-white truncate">{tag.label}</div>
        ...
        {showCreateNew && (
          <div className="mt-2 bg-gray-700 rounded-lg border border-dashed border-gray-600 p-3">
            <div className="text-sm text-gray-300 mb-2">
              {t.tags.createNew}: <span className="font-semibold text-white">#{searchQuery}</span>
            </div>
            <div className="flex items-center gap-2">
              <select
              ...
              >
                {Object.entries(TAG_CATEGORY_CONFIG).map(
                  ([key, config]): JSX.Element => (
      ...
      <div className="px-4 py-3 border-t border-gray-700 bg-gray-900/30 flex items-center justify-between">
        <div className="text-xs text-gray-400">
          {existingTags.length} tag{existingTags.length !== 1 ? 's' : ''} applied
        </div>
        <button
          onClick={onClose}
