    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-gray-800 rounded-lg shadow-2xl border border-gray-600 w-full max-w-4xl max-h-[80vh] flex flex-col">
        ...
        <div className="px-6 py-4 border-b border-gray-700 flex items-center justify-between">
          <h2 className="text-xl font-semibold text-white">Thread Configuration</h2>
          <button
            onClick={onClose}
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-sm text-gray-400">{selectedModelKeys.size} selected</span>
              </div>
                  ...
                  <div className="text-xs text-gray-300">
                    {currentDomain === 'practice' ? (
                      <span>
                        ...
                        <strong className="text-gray-200">Practice Areas</strong> (legal)
                      </span>
                    ) : (
                      <span>
                        📊 Showing models configured for{' '}
                        <strong className="text-gray-200">Advisory Areas</strong> (business)
                      </span>
                    )}
                  </div>
                </div>
              ...
              <div className="text-xs text-gray-400 mb-3">
                Select one or more models to get diverse opinions and reduce hallucinations:
              </div>
              <div className="space-y-2 overflow-y-auto pr-2">
                {getAllAvailableModels(config.providers, providerTemplates, currentDomain).map(
                  (model): JSX.Element => {
                    ...
                    return (
                      <button
                        key={key}
                        onClick={() => toggleModelSelection(model.providerId, model.modelId)}
                      ...
                      >
                        <div className="flex items-center justify-between mb-1">
                          ...
                          {isSelected && <span className="text-gray-400 text-xs">✓ Selected</span>}
                        </div>
                        ...
                        <div className="flex items-center gap-2 mt-2 ml-7">
                          ...
                          {model.maxContextWindow <= 16384 && (
                            ...
                            >
                              ...
                              {(model.maxContextWindow / 1024).toFixed(0)}K context
                            </span>
                          )}
                        </div>
                      </button>
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-medium text-white">Jurisdictions</h3>
              ...
              <div className="text-xs text-gray-400 mb-3">
                Select specific jurisdictions to focus legal analysis, or leave empty for global
                perspective:
              </div>
              <div className="space-y-2">
                {JURISDICTIONS.map((jurisdiction): JSX.Element => {
                  const isSelected = selectedJurisdictions.has(jurisdiction.code);
                  return (
                    <button
