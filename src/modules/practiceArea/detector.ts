...
export function detectPracticeArea(
    ...
    const result = detectPracticeAreaWithConfidence(text, settings);
    return result.area;
}
...
export function detectPracticeAreaWithConfidence(
    ...
    const scores = DEFAULT_PRACTICE_AREAS
    ...
    if (!bestMatch || bestMatch.confidence < config.minConfidence) {
        const generalArea = DEFAULT_PRACTICE_AREAS.find(a => a.id === 'general')!;
        return {
            area: generalArea,
            confidence: 0,
            matchCount: 0,
...
export function getPracticeAreaById(id: string): LegalPracticeArea | undefined {
    return DEFAULT_PRACTICE_AREAS.find(area => area.id === id);
}

/**
 * Get practice areas that match any of the provided keywords
 */
export function searchPracticeAreasByKeyword(keyword: string): LegalPracticeArea[] {
    const lowerKeyword = keyword.toLowerCase();

    return DEFAULT_PRACTICE_AREAS.filter(area =>
