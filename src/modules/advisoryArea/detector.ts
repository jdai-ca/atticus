...
function scoreAdvisoryArea(
  ...
  const totalMatches =
    phraseMatchCount * 2 + exactMatchCount + synonymMatchCount * 0.8 + stemMatchCount * 0.5;

  // Calculate confidence score with improved algorithm
  const keywordCoverage = area.keywords.length > 0 ? totalMatches / area.keywords.length : 0;
  ...
  const confidence = Math.min(
    1,
    keywordCoverage * 0.4 + matchDensity * 0.3 + phraseBonus * 0.2 + positionScore * 0.1
  );
...
export function detectAdvisoryAreaWithConfidence(
  ...
  const areas =
    advisoryAreas ||
    advisoryAreaManager.getAllAreas().map(
      (a): LegalPracticeArea =>
        ({
          id: a.id,
          name: a.name,
  ...
  if (!bestMatch || bestMatch.confidence < config.minConfidence) {
    const generalArea = areas.find((a): boolean => a.id === 'general-advisory') || {
      ...
      description: 'General business consulting',
      systemPrompt:
        'You are a business advisory AI assistant providing general consulting guidance.',
      color: '#1e40af',
    };
    return {
      area: generalArea,
