...
export const JURISDICTIONS: JurisdictionInfo[] = [
  {
    ...
    flag: '🇨🇦',
    description:
      'Canadian federal and provincial law, including interprovincial trade and regulatory differences',
    coverage: 80,
  },
  {
    code: 'US',
    ...
    flag: '🇺🇸',
    description:
      'U.S. federal and state law, including interstate commerce and jurisdictional conflicts',
    coverage: 92,
  },
  {
    code: 'MX',
    ...
    flag: '🇲🇽',
    description:
      'Mexican federal and state law, including interstate commerce and regulatory variations',
    coverage: 70,
  },
  {
    code: 'EU',
...
/**
 * Get internal complexity guidance for a specific jurisdiction
 */
function getInternalComplexityGuidance(code: Jurisdiction): string {
...
/**
 * Get combination guidance for multi-jurisdiction queries
 */
function getCombinationGuidance(jurisdictions: Jurisdiction[]): string {
  let guidance = '';

  // Check for CUSMA countries (any combination of CA, US, MX)
  const cusmaCountries = jurisdictions.filter((j): boolean => ['CA', 'US', 'MX'].includes(j));
  ...
  // Add jurisdiction-specific complexities
...
export function getJurisdictionSystemPromptAppendix(jurisdictions: Jurisdiction[]): string {
  ...
  const isCUSMA =
    jurisdictions.includes('CA') && jurisdictions.includes('US') && jurisdictions.includes('MX');
  ...
  const combinationGuidance = getCombinationGuidance(jurisdictions);
