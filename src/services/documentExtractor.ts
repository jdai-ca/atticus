...
if (typeof process !== 'undefined' && process.versions?.electron) {
  ...
  if (isDev) {
    ...
    logger.debug('PDF.js worker configured for dev', {
    ...
    pdfjsLib.GlobalWorkerOptions.workerSrc = `file://${workerPath}`;
    logger.debug('PDF.js worker configured for production', {
      workerSrc: pdfjsLib.GlobalWorkerOptions.workerSrc,
    });
  }
  ...
  logger.debug('PDF.js worker configured for renderer', {
    workerSrc: pdfjsLib.GlobalWorkerOptions.workerSrc,
  });
}
 ...
 */
export async function extractDocumentText(attachment: Attachment): Promise<ExtractedDocument> {
  ...
  switch (ext) {
    ...
    case '.txt':
      return await extractFromText(attachment.data);
    default:
