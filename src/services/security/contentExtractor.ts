...
async function extractPDF(buffer: Buffer): Promise<{
  ...
  try {
    ...
    warnings.push(
      `PDF extraction error: ${error instanceof Error ? error.message : String(error)}`
    );
    return { text: '', metadata: {}, images: [], warnings };
  }
}
...
async function extractDOCX(buffer: Buffer): Promise<{
  ...
  try {
    ...
    let text = '';
    if (xmlMatches) {
      text = xmlMatches.map(match => match.replace(/<[^>]+>/g, '')).join(' ');
    }
    ...
    warnings.push(
      `DOCX extraction error: ${error instanceof Error ? error.message : String(error)}`
    );
    return { text: '', metadata: {}, warnings };
  }
}
...
async function extractXLSX(buffer: Buffer): Promise<{
  ...
  try {
    ...
    let text = '';
    if (cellMatches) {
      text = cellMatches.map(match => match.replace(/<[^>]+>/g, '')).join(' ');
    }
    ...
    warnings.push(
      `XLSX extraction error: ${error instanceof Error ? error.message : String(error)}`
    );
    return { text: '', metadata: {}, warnings };
  }
}
