...
export async function generateSecurityReport(data: {
  ...
  const executiveSummary = generateExecutiveSummary(file, findings, riskAssessment);
...
function generateExecutiveSummary(
  ...
  switch (riskAssessment.severity) {
    case 'critical':
      summary +=
        '🚨 CRITICAL THREAT LEVEL: This file poses severe security risks and must be blocked immediately. ';
      summary +=
        'Do not process this file with any systems. Notify security team and legal counsel.\n\n';
      break;
    case 'high':
      summary +=
        '⚠️  HIGH THREAT LEVEL: This file contains significant security concerns requiring immediate human review. ';
    ...
    case 'medium':
      ...
      summary +=
        'Review redaction suggestions and consider manual inspection before proceeding.\n\n';
      break;
    case 'low':
      summary +=
        '✅ LOW THREAT LEVEL: This file appears safe to process with standard data handling procedures.\n\n';
      break;
  }
 ...
 */
export function calculateReportingDeadline(violations: ComplianceViolation[]): Date | undefined {
...
export function exportReportAsText(report: SecurityReport): string {
  ...
  if (report.complianceViolations.length > 0) {
    ...
    report.complianceViolations.forEach((v: (typeof report.complianceViolations)[number]): void => {
  ...
  if (report.redactionSuggestions.length > 0) {
    ...
    report.redactionSuggestions
      .slice(0, 10)
      .forEach((s: (typeof report.redactionSuggestions)[number], i: number): void => {
