...
export function calculateRiskScore(findings: {
  ...
  const manipulationRisk = calculateManipulationRisk(findings.adversarialFindings);
...
function calculatePIIRisk(findings: PIIFinding[]): RiskDimension {
  ...
  const hasIdentityProfile = findings.some((f): boolean =>
    f.context.includes('Complete identity profile')
  );
