...
export class AuditLogger {
    ...
    private resolveMaxEntriesPerConversation(maxEntries?: number): number {
        const runtimeConfiguredMax = (
            globalThis as typeof globalThis & AtticusAuditRuntimeConfig
        ...
        return Math.max(
            MIN_MAX_ENTRIES_PER_CONVERSATION,
            Math.floor(sourceMaxEntries as number),
        );
    }
    ...
    async exportForEDiscovery(conversationId?: string): Promise<string> {
        try {
            ...
            const jsonLines = logs.flatMap((log): string[] =>
                log.entries.map((entry): string => JSON.stringify({
                    ...entry,
    ...
    private async storeEvent(entry: AuditLogEntry, conversationId?: string): Promise<void> {
        ...
        try {
            ...
            const appendResult = await window.electronAPI.auditLogAppend(conversationId, JSON.stringify(entry));
    ...
    private getApplicationVersion(): string {
        return (globalThis as typeof globalThis & { __ATTICUS_VERSION__?: string }).__ATTICUS_VERSION__ || '0.9.21';
    }
...
export function isPIIScanDetails(details: unknown): details is PIIScanAuditDetails {
    return !!details && typeof details === 'object' && 'scanResult' in details && 'messagePreview' in details;
}
