...
class PracticeConfigLoader {
  ...
  private getCacheKey(language: Language) {
    return `practice-config-${language}`;
  }
  private getVersionKey(language: Language) {
    return `practice-config-version-${language}`;
  }
  ...
  private async updateFromRemote(currentVersion: string, language: Language = 'en'): Promise<void> {
    try {
      ...
      if (!this.isCompatibleVersion(config)) {
        logger.warn('Remote config requires newer app version', {
          minRequired: config.minAppVersion,
        });
        return;
      }
  ...
  private notifyConfigUpdate(newVersion: string): void {
    ...
    (globalThis as typeof globalThis & { dispatchEvent: (event: Event) => boolean }).dispatchEvent(
      new CustomEvent('practice-config-updated', {
      ...
      practiceAreas: [
        {
          ...
          systemPrompt:
            'You are a helpful legal AI assistant providing general legal guidance. Always remind users to consult with licensed attorneys for specific legal advice.',
          color: '#6b7280',
        },
      ],
    };
