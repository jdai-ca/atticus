...
export async function analyzeFile(file: UploadedFile): Promise<SecurityAnalysisResult> {
  ...
  try {
    ...
    const piiFindings = await detectPII(extractedContent.text, extractedContent.metadata);

    // Phase 3B: AI Countermeasure Detection
    const adversarialFindings = await detectAdversarialPatterns(extractedContent.text);
    ...
    const obfuscationFindings = await detectObfuscation(extractedContent.text);
