...
export abstract class BaseProviderAdapter implements ProviderAdapter {
  ...
  async sendRequest(request: ChatRequest): Promise<ChatResponse> {
    ...
    // Determine intent to target localhost from the parsed hostname (not a naive substring
    // match, which a crafted path/query like "http://192.168.1.5/localhost" could exploit
    // to bypass the internal-network SSRF guard in validateEndpoint).
    const isLocalhostEndpoint = ((): boolean => {
      try {
    ...
    validateEndpoint(endpoint, isLocalhostEndpoint);
