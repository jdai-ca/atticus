...
export function isImageFile(
  fileOrAttachment: string | { extension?: string; type?: string; name: string }
): boolean {
 ...
 * MUST be called in renderer process (requires DOM APIs)
 */
async function convertPDFToImagesForOpenAI(
  base64Data: string,
  maxPages?: number
): Promise<string[]> {
  try {
    ...
    try {
      annotationStorage =
        (pdf as pdfjsLib.PDFDocumentProxy & { annotationStorage?: unknown }).annotationStorage ??
        null;
    ...
    for (let pageNum = 1; pageNum <= pagesToRender; pageNum++) {
      ...
      if (imageData && imageData.length > 0) {
        ...
        logger.warn('Skipping empty PDF page for OpenAI/xAI', '[Multimodal Formatter]', {
          pageNum,
        });
      }
    }

    logger.info('PDF converted for OpenAI/xAI', '[Multimodal Formatter]', {
 ...
 * MUST be called in renderer process (requires DOM APIs)
 */
async function convertPDFToImagesForGoogle(
  base64Data: string,
  maxPages?: number
): Promise<string[]> {
  try {
    ...
    logger.info('PDF converted for Google Gemini', '[Multimodal Formatter]', {
      imageCount: images.length,
    });
    return images;
  } catch (error) {
...
export async function convertPDFToImages(base64Data: string, maxPages?: number): Promise<string[]> {
  logger.warn(
    'convertPDFToImages called without provider - defaulting to OpenAI optimization',
  ...
  for (const attachment of msg.attachments ?? []) {
    ...
    if (isImageFile(attachment)) {
      ...
      if (typeof document !== 'undefined') {
        logger.info(
          `Converting PDF to images for ${providerLabel} vision`,
          '[Multimodal Formatter]',
          {
            fileName: attachment.name,
          }
        ...
        logger.warn(
          'PDF conversion skipped (main process), extracting text',
          '[Multimodal Formatter]',
          {
            fileName: attachment.name,
          }
      ...
      if (typeof document !== 'undefined') {
        logger.info(
          `Converting Word document to images for ${providerLabel} vision`,
          '[Multimodal Formatter]',
          {
            fileName: attachment.name,
          }
        ...
        logger.warn(
          'Word conversion skipped (main process), extracting text',
          '[Multimodal Formatter]',
          {
            fileName: attachment.name,
          }
 ...
 */
async function formatWithOpenAIStyle(
  messages: Message[],
  providerName: string
  ...
  for (const msg of messages) {
    if (!msg.attachments || msg.attachments.length === 0) {
      ...
      formattedMessages.push({
        role: msg.role,
        content: await buildOpenAIStyleContent(msg, providerName),
      });
    }
  }
...
export async function formatForAnthropic(messages: Message[]): Promise<AnthropicWireMessage[]> {
  ...
  for (const msg of messages) {
    ...
    const content: AnthropicWireContent[] = [{ type: 'text', text: msg.content }];
...
export async function formatForGemini(messages: Message[]): Promise<GeminiContent[]> {
  ...
  for (const msg of messages) {
    ...
    // Process attachments
    const attachmentParts = await processAttachments<GeminiPart>(
      msg,
      'Gemini',
      {
        onImage: (mimeType, data) => {
    ...
    const validParts = parts.filter((part, index): boolean => {
      ...
      if (part.inlineData) {
        ...
        if (!isValid) {
          logger.error(
            'CRITICAL: Filtered out invalid inlineData part for Google',
            '[Multimodal Formatter]',
            {
              partIndex: index,
  ...
  for (let i = 0; i < formattedMessages.length; i++) {
    ...
    for (let j = 0; j < msg.parts.length; j++) {
      ...
      if (part.inlineData && (!part.inlineData.data || part.inlineData.data.length === 0)) {
        logger.error(
          'FINAL CHECK FAILED: Invalid part found in formatted messages',
          '[Multimodal Formatter]',
          {
            messageIndex: i,
            partIndex: j,
  ...
  logger.info('Gemini format validation complete', '[Multimodal Formatter]', {
    ...
    totalParts: formattedMessages.reduce((sum: number, m): number => sum + m.parts.length, 0),
  });
 ...
 */
export async function extractTextFromDocument(attachment: Attachment): Promise<string | null> {
  ...
  try {
    ...
    if (extracted.error) {
      logger.warn(
        '[Document Extraction] Extraction failed',
    ...
    logger.error(
      '[Document Extraction] Unexpected extraction error',
