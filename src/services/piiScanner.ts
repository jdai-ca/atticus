...
const PII_PATTERNS: PIIPatternConfig[] = [
  ...
  // Credit Card Numbers - Visa (starts with 4)
  {
    ...
    pattern: /\b4\d{3}[-\s]?\d{4}[-\s]?\d{4}[-\s]?\d{4}\b|\b4\d{12}(?:\d{3})?\b/g,
    riskLevel: RiskLevel.CRITICAL,
  ...
  {
    type: PIIType.CREDIT_CARD,
    pattern: /\b5[1-5]\d{2}[-\s]?\d{4}[-\s]?\d{4}[-\s]?\d{4}\b|\b5[1-5]\d{14}\b/g,
    riskLevel: RiskLevel.CRITICAL,
  ...
  {
    type: PIIType.CREDIT_CARD,
    pattern: /\b3[47]\d{2}[-\s]?\d{6}[-\s]?\d{5}\b|\b3[47]\d{13}\b/g,
    riskLevel: RiskLevel.CRITICAL,
  ...
  {
    type: PIIType.CREDIT_CARD,
    pattern: /\b3(?:0[0-5]|[68]\d)\d[-\s]?\d{6}[-\s]?\d{4}\b|\b3(?:0[0-5]|[68]\d)\d{11}\b/g,
    riskLevel: RiskLevel.CRITICAL,
  ...
  {
    type: PIIType.CREDIT_CARD,
    pattern: /\b6(?:011|5\d{2})[-\s]?\d{4}[-\s]?\d{4}[-\s]?\d{4}\b|\b6(?:011|5\d{2})\d{12}\b/g,
    riskLevel: RiskLevel.CRITICAL,
  ...
  // Credit Card Numbers - JCB (starts with 2131, 1800, or 35)
  {
    type: PIIType.CREDIT_CARD,
    pattern:
      /\b(?:2131|1800|35\d{3})[-\s]?\d{4}[-\s]?\d{4}[-\s]?\d{4}\b|\b(?:2131|1800|35\d{3})\d{11}\b/g,
    riskLevel: RiskLevel.CRITICAL,
  ...
  // API Keys (common formats) - UNIVERSAL
  {
    ...
    pattern:
      /(?:api[_-]?key|apikey|access[_-]?token|secret[_-]?key)[\s:=]+["']?([\w-]{20,})["']?/gi,
    riskLevel: RiskLevel.CRITICAL,
  ...
  // Full Names - UNIVERSAL
  {
    ...
    pattern:
      /\b(?:Mr\.|Mrs\.|Ms\.|Dr\.|Prof\.|Sr\.|Sra\.|Dr\.a)\s+[A-Z][a-z]+(?:\s+[A-Z][a-z]+){1,3}\b/g,
    riskLevel: RiskLevel.MODERATE,
...
export class PIIScanner {
  ...
  private isPatternApplicable(
    patternConfig: (typeof PII_PATTERNS)[0],
    jurisdictions?: Jurisdiction[]
  ): boolean {
    if (!jurisdictions || jurisdictions.length === 0) {
      return true;
    }
    return patternConfig.jurisdictions.some((j: Jurisdiction): boolean =>
   ...
   */
  private findMatchedJurisdiction(
    patternConfig: (typeof PII_PATTERNS)[0],
    jurisdictions?: Jurisdiction[]
    ...
    return patternConfig.jurisdictions.find((j: Jurisdiction): boolean =>
      jurisdictions.includes(j)
    );
  }
  ...
  private processPattern(
    ...
    while ((match = patternConfig.pattern.exec(text)) !== null) {
      if (!this.validateMatch(patternConfig.type, match[0], text)) {
        continue;
      }
  ...
  private calculateOverallRisk(findings: PIIDetection[]): RiskLevel {
    ...
    const hasHigh = findings.some((f: PIIDetection): boolean => f.riskLevel === RiskLevel.HIGH);
  ...
  generateWarningMessage(result: PIIScanResult): string {
    ...
    const uniqueRecs = new Set(result.findings.map((f: PIIDetection): string => f.recommendation));
